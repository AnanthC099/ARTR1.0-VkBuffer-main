...
*/
#include <SPIRV/GlslangToSpv.h> //C:\VulkanSDK\Vulkan\Include\glslang\SPIRV (Uptil 1.3.268 Vulkan SDK version)
#include <Public/ShaderLang.h> //C:\VulkanSDK\Vulkan\Include\glslang\Public : Started needing recently (For 1.4.304 Vulkan SDK version)
...
/*
GLSLANG/SPIRV related libraries for Vulkan SDk 1.4.304
Main rada ithe aahe
*/
#pragma comment(lib, "glslang.lib") //Started needing recently. Before we needed spirv.lib, before that genericcodegen.lib , before that machine independent.lib
#pragma comment(lib, "SPIRV-tools.lib") //This is required since begining
...
VkResult CreateShaders(void)
{
	...
	*/
	...
	if(FALSE == bDone)
	{
		fprintf(gFILE, "CreateShaders(): glsl2SPV() failed for vertex shader\n");
		vkResult = VK_ERROR_INITIALIZATION_FAILED;
		return vkResult;
	}
	else
	{
		fprintf(gFILE, "CreateShaders(): glsl2SPV() suceeded for vertex shader\n");
	}
	...
	*/
	...
	vkShaderModuleCreateInfo.pCode = (uint32_t*)vertex_spv.data();
	...
	*/
	...
	if(FALSE == bDone)
	{
		fprintf(gFILE, "CreateShaders(): glsl2SPV() failed for fragment shader\n");
		vkResult = VK_ERROR_INITIALIZATION_FAILED;
		return vkResult;
	}
	else
	{
		fprintf(gFILE, "CreateShaders(): glsl2SPV() suceeded for fragment shader\n");
	}
...
BOOL glsl2SPV(const VkShaderStageFlagBits shader_type, const char* pShader, vector<unsigned int>& spv)
{
	...
	*/
	...
	/*
	Find the shader language in form of EShLanguage
	*/
	...
	*/
	...
	/*
	Initialize builtinShader resources
	*/
	...
	*/
	...
	{
		fprintf(gFILE, "glsl2SPV(): parse() method failed with log %s\n",shader.getInfoLog());
		fprintf(gFILE, "glsl2SPV(): parse() method failed with debug log %s\n",shader.getInfoDebugLog());
		return FALSE;
	}
	else
	{
		fprintf(gFILE, "glsl2SPV(): parse() method succeeded\n");
	}
	...
	*/
	...
	{
		fprintf(gFILE, "glsl2SPV(): link() method failed with log %s\n",shader.getInfoLog());
		fprintf(gFILE, "glsl2SPV(): link() method failed with debug log %s\n",shader.getInfoDebugLog());
		return FALSE;
	}
	else
	{
		fprintf(gFILE, "glsl2SPV(): link() method succeeded\n");
	}
	
	/*
	Finally now set this whole cooked program and SPIRV code to glslang library
	*/
...
/*
//https://github.com/KhronosGroup/glslang/blob/main/SPIRV/GlslangToSpv.cpp
//https://registry.khronos.org/vulkan/specs/latest/man/html/VkShaderStageFlagBits.html
*/
