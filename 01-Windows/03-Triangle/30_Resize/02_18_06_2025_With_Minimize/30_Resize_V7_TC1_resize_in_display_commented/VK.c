		...
		else
		{
			if (gbActive == TRUE)
			{
				if(bWindowMinimize == FALSE)
				{
					...
					update();
					/* //VK_ERROR_OUT_OF_DATE_KHR and VK_SUBOPTIMAL_KHR are meant for future issues.You can remove them when using in real world scenarios
					if ((vkResult != VK_FALSE) && (vkResult != VK_SUCCESS)) //VK_ERROR_OUT_OF_DATE_KHR and VK_SUBOPTIMAL_KHR are meant for future issues.You can remove them.
					{
						fprintf(gFILE, "WinMain(): display() function failed\n");
						bDone = TRUE;
					}
...
VkResult display(void)
{
	...
	*/
	...
	if(vkResult != VK_SUCCESS) 
	{
		if((vkResult == VK_ERROR_OUT_OF_DATE_KHR) || (vkResult == VK_SUBOPTIMAL_KHR))
		{
			...
			fprintf(gFILE, "display(): vkAcquireNextImageKHR() needs to recreate swapchain again\n");
		}
		else
		{
	...
	*/
	...
	if(vkResult != VK_SUCCESS)
	{
		...
		{
			//resize(winWidth, winHeight);
			fprintf(gFILE, "display(): vkQueuePresentKHR() needs to recreate swapchain again\n");
		}
		else
		{
