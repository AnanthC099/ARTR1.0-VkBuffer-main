...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	switch (iMsg)
	{
		...
		case WM_SIZE:
		{
			...
			{
				// Window is iconic → do NOT recreate swap‑chain.
				// Optionally set a flag so your render loop can skip work.
				return 0;
			}

			// Ignore bogus 0×0 coming from some transient states.
			if (width == 0 || height == 0)
				return 0;
...
static VkExtent2D chooseSwapExtent(const VkSurfaceCapabilitiesKHR *caps)   /* pass pointer, not ref */
{
    /* Case 1 – the surface dictates the size (most windowed WSI paths) */
    if ( (caps->currentExtent.width  != UINT32_MAX) && (caps->currentExtent.width  != 0) && (caps->currentExtent.height != 0))
    {
        return caps->currentExtent;   /* ready to use */
    }

    /* Case 2 – we must choose the size ourselves */
    VkExtent2D actual;
    actual.width  = (uint32_t)winWidth;
    actual.height = (uint32_t)winHeight;

    /* clamp to the limits advertised by the surface --------------------- */
    if (actual.width < caps->minImageExtent.width)
        actual.width = caps->minImageExtent.width;
    else if (actual.width > caps->maxImageExtent.width)
    ...
    else if (actual.height > caps->maxImageExtent.height)
...
VkResult CreateSwapChain(VkBool32 vsync)
{
	...
	*/
	...
	vkExtent2D_SwapChain = chooseSwapExtent(&vkSurfaceCapabilitiesKHR);
    fprintf(gFILE, "CreateSwapChain(): final extent = %u × %u\n", vkExtent2D_SwapChain.width, vkExtent2D_SwapChain.height);
