...
// ----- ADDED CODE: Include GLM (for perspective matrix) -----
#include "glm/glm.hpp"
...
// ----- Helper to load SPIR-V from file into a VkShaderModule -----
...
// ----- Create a Pipeline Layout (with push constants for a mat4) -----
...
static VkResult CreateGraphicsPipeline(void)
{
    ...
    // 1) Load SPIR-V modules (we assume they read a vec3 at location=0 in the vert shader)
    ...
    // ***** CHANGED: Vertex Input State for (x, y, z)
    VkVertexInputBindingDescription bindingDesc;
    bindingDesc.binding   = 0;
    bindingDesc.stride    = sizeof(float) * 3; // 3 floats => x,y,z
    bindingDesc.inputRate = VK_VERTEX_INPUT_RATE_VERTEX;

    VkVertexInputAttributeDescription attrDesc;
    attrDesc.binding  = 0;                         // matches bindingDesc.binding
    attrDesc.location = 0;                         // "layout(location=0)" in vert
...
// ----- Build Command Buffers: now uses perspective -----
static VkResult buildCommandBuffers(void)
{
    ...
    for(uint32_t i=0; i<swapchainImageCount; i++){
        ...
        {
            // Keep the flipped Y for Vulkan
            VkViewport vp;
            vp.x        = 0.0f;
            vp.y        = (float)vkExtent2D_SwapChain.height; // start at bottom
            vp.width    = (float)vkExtent2D_SwapChain.width;
            vp.height   = -(float)vkExtent2D_SwapChain.height; // negative
            vp.minDepth = 0.0f;
            vp.maxDepth = 1.0f;
        ...
        // ***** CHANGED: push perspective instead of ortho
        {
            ...
            vkCmdPushConstants(
                ...
                &perspective[0][0]     // pointer
            );
        }

        // Bind vertex buffer and draw the triangle
        {
...
VkResult initialize(void)
{
    ...
    // Create vertex buffer with our (x,y,z) data
    r=CreateVertexBuffer();
    if(r!=VK_SUCCESS) return r;

    // Create pipeline layout + pipeline (dynamic rendering)
    ...
    // Build command buffers (which draw our triangle)
    r=buildCommandBuffers();
    if(r!=VK_SUCCESS) return r;
...
void uninitialize(void)
{
    ...
    if(vkDevice){
        ...
        // Destroy pipeline, layout, vertex buffer, etc.
        if(gPipeline) {
...
int WINAPI WinMain(HINSTANCE hInst,HINSTANCE hPrev,LPSTR lpszCmdLine,int iCmdShow)
{
    ...
    ghwnd=CreateWindowEx(
        WS_EX_APPWINDOW,
        gpszAppName,
        TEXT("VulkanApp - Perspective Triangle with Dynamic Rendering"),
