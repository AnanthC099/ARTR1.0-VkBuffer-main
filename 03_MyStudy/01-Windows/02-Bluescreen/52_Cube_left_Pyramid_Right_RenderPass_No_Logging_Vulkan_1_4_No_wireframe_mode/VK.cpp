...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow) {
    ...
    ghwnd = CreateWindowEx(
        WS_EX_APPWINDOW,
        gpszAppName,
        TEXT("Vulkan 3D Two Objects (Solid)"),
...
static VkResult CreateVulkanInstance() {
    VkResult r=FillInstanceExtensionNames();
    if(r!=VK_SUCCESS) return r;

    r=FillInstanceLayerNames();
    if(r!=VK_SUCCESS) return r;
...
static VkResult CreateLogicalDeviceAndQueue() {
    // We remove fillModeNonSolid usage to disable wireframe mode
    VkResult r=FillDeviceExtensionNames();
    if(r!=VK_SUCCESS) return r;
    ...
    qInfo.queueCount=1;
    qInfo.pQueuePriorities=&qp;

    // No special features needed for solid mode, so pass null or empty struct
    VkDeviceCreateInfo ci{};
    ci.sType=VK_STRUCTURE_TYPE_DEVICE_CREATE_INFO;
    ci.queueCreateInfoCount=1;
    ci.pQueueCreateInfos=&qInfo;
    ci.enabledExtensionCount=enabledDeviceExtensionsCount;
    ci.ppEnabledExtensionNames=enabledDeviceExtensionNames_array;

    // Key change: do NOT enable fillModeNonSolid; pass NULL if no features needed
...
static VkFormat findSupportedFormat(const std::vector<VkFormat> &candidates,VkImageTiling tiling,VkFormatFeatureFlags feats){
    for(VkFormat f: candidates){
        ...
        vkGetPhysicalDeviceFormatProperties(vkPhysicalDevice_sel,f,&p);
...
VkResult CreateDepthResources() {
    ...
    for(uint32_t i=0;i<swapchainImageCount;i++){
        ...
        if(vkCreateImage(vkDevice,&ii,nullptr,&gDepthImages[i])!=VK_SUCCESS)
            return VK_ERROR_INITIALIZATION_FAILED;

        VkMemoryRequirements mr;
        vkGetImageMemoryRequirements(vkDevice,gDepthImages[i],&mr);
        ...
        if(vkAllocateMemory(vkDevice,&ai,nullptr,&gDepthImagesMemory[i])!=VK_SUCCESS)
            return VK_ERROR_INITIALIZATION_FAILED;
...
VkResult CreateRenderPass() {
    ...
    subp.pColorAttachments=&colorRef;
...
static uint32_t FindMemoryTypeIndex(uint32_t typeFilter, VkMemoryPropertyFlags props) {
    for(uint32_t i=0;i<vkPhysicalDeviceMemoryProperties.memoryTypeCount;i++){
        if((typeFilter&(1<<i)) &&
           (vkPhysicalDeviceMemoryProperties.memoryTypes[i].propertyFlags & props)==props)
            return i;
    }
    return UINT32_MAX;
}
VkResult CreateCubeVertexBuffer() {
    ...
    if(vkCreateBuffer(vkDevice,&bi,nullptr,&gVertexBufferCube)!=VK_SUCCESS)
        return VK_ERROR_INITIALIZATION_FAILED;

    VkMemoryRequirements mr;
    vkGetBufferMemoryRequirements(vkDevice,gVertexBufferCube,&mr);
    ...
    if(vkAllocateMemory(vkDevice,&ai,nullptr,&gVertexBufferMemoryCube)!=VK_SUCCESS)
        return VK_ERROR_INITIALIZATION_FAILED;
...
VkResult CreatePyramidVertexBuffer() {
    ...
    if(vkCreateBuffer(vkDevice,&bi,nullptr,&gVertexBufferPyramid)!=VK_SUCCESS)
        return VK_ERROR_INITIALIZATION_FAILED;

    VkMemoryRequirements mr;
    vkGetBufferMemoryRequirements(vkDevice,gVertexBufferPyramid,&mr);
    ...
    if(vkAllocateMemory(vkDevice,&ai,nullptr,&gVertexBufferMemoryPyramid)!=VK_SUCCESS)
        return VK_ERROR_INITIALIZATION_FAILED;
...
VkResult CreateUniformBufferCube() {
    ...
    if(vkCreateBuffer(vkDevice,&bi,nullptr,&gUniformBufferCube)!=VK_SUCCESS)
        return VK_ERROR_INITIALIZATION_FAILED;

    VkMemoryRequirements mr;
    vkGetBufferMemoryRequirements(vkDevice,gUniformBufferCube,&mr);
    ...
    if(vkAllocateMemory(vkDevice,&ai,nullptr,&gUniformBufferMemoryCube)!=VK_SUCCESS)
        return VK_ERROR_INITIALIZATION_FAILED;
...
VkResult CreateUniformBufferPyramid() {
    ...
    if(vkCreateBuffer(vkDevice,&bi,nullptr,&gUniformBufferPyramid)!=VK_SUCCESS)
        return VK_ERROR_INITIALIZATION_FAILED;

    VkMemoryRequirements mr;
    vkGetBufferMemoryRequirements(vkDevice,gUniformBufferPyramid,&mr);
    ...
    if(vkAllocateMemory(vkDevice,&ai,nullptr,&gUniformBufferMemoryPyramid)!=VK_SUCCESS)
        return VK_ERROR_INITIALIZATION_FAILED;
...
VkResult CreateDescriptorSets() {
    ...
    if(vkAllocateDescriptorSets(vkDevice,&ai,sets.data())!=VK_SUCCESS)
        return VK_ERROR_INITIALIZATION_FAILED;

    gDescriptorSetCube=sets[0];
    gDescriptorSetPyramid=sets[1];

    {
        VkDescriptorBufferInfo bi{};
        bi.buffer=gUniformBufferCube;
        bi.offset=0;
        bi.range=sizeof(UniformBufferObject);

        VkWriteDescriptorSet wd{};
        wd.sType=VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
        wd.dstSet=gDescriptorSetCube;
        wd.dstBinding=0;
    ...
    {
        VkDescriptorBufferInfo bi{};
        bi.buffer=gUniformBufferPyramid;
        bi.offset=0;
        bi.range=sizeof(UniformBufferObject);

        VkWriteDescriptorSet wd{};
        wd.sType=VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
        wd.dstSet=gDescriptorSetPyramid;
        wd.dstBinding=0;
        wd.descriptorType=VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
        wd.descriptorCount=1;
        wd.pBufferInfo=&bi;
...
VkResult CreateGraphicsPipeline(){
    ...
    vii.pVertexBindingDescriptions=&bd;
    vii.vertexAttributeDescriptionCount=(uint32_t)ads.size();
    vii.pVertexAttributeDescriptions=ads.data();
    ...
    vsci2.viewportCount=1;
    vsci2.pViewports=&vp;
    vsci2.scissorCount=1;
    vsci2.pScissors=&scissor;

    // <-- Key change for solid rendering
    VkPipelineRasterizationStateCreateInfo rsci{};
    rsci.sType=VK_STRUCTURE_TYPE_PIPELINE_RASTERIZATION_STATE_CREATE_INFO;
    rsci.polygonMode=VK_POLYGON_MODE_FILL; // solid fill
    rsci.cullMode=VK_CULL_MODE_NONE;       // or VK_CULL_MODE_BACK_BIT if desired
