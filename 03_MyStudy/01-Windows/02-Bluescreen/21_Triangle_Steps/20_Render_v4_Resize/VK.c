...
static VkResult recreateSwapChain(void)
{
    ...
    // We'll just replicate what 'initialize' did for the swap chain section:
    
    // NOTE: The device still exists, so we skip instance & device creation.
...
/////////////////////////////////////////////////////////////////
// --- INSTANCE LAYER AND EXTENSION SETUP (unchanged) --- //
/////////////////////////////////////////////////////////////////
static VkResult FillInstanceLayerNames(void)
{
    ...
    if (vkResult != VK_SUCCESS)
    {
        fprintf(gFILE, "FillInstanceLayerNames(): Could not enumerate instance layers\n");
        free(layerProps);
        return vkResult;
    }
...
VkResult CreateVulkanInstance(void)
{
    ...
    if(enabledInstanceLayerCount > 0)
    {
...
void uninitialize(void)
{
    ...
    if(vkDevice)
    {
        ...
        // *** NEW/CHANGED FOR RESIZE ***
        // Instead of directly destroying below, call cleanupSwapChain
        // to clean up any remaining swapchain resources.
        cleanupSwapChain();
