...
HWND ghwnd=NULL; BOOL gbActive=FALSE; DWORD dwStyle=0; WINDOWPLACEMENT wpPrev;
BOOL gbFullscreen=FALSE; FILE* gFILE=NULL; const char* gpszAppName="VulkanApp";
...
VkExtent2D vkExtent2D_SwapChain; uint32_t swapchainImageCount=0;
VkImage* swapChainImage_array=NULL; VkImageView* swapChainImageView_array=NULL;
VkCommandPool vkCommandPool=VK_NULL_HANDLE;
VkCommandBuffer* vkCommandBuffer_array=NULL;
VkSemaphore imageAvailableSemaphores[MAX_FRAMES_IN_FLIGHT];
VkSemaphore renderFinishedSemaphores[MAX_FRAMES_IN_FLIGHT];
VkFence inFlightFences[MAX_FRAMES_IN_FLIGHT];
uint32_t currentFrame=0; uint32_t currentImageIndex=UINT32_MAX;
BOOL bInitialized=FALSE; VkClearColorValue vkClearColorValue;
...
LRESULT CALLBACK WndProc(HWND hwnd,UINT iMsg,WPARAM wParam,LPARAM lParam)
{
    switch(iMsg)
    {
        ...
        case WM_SIZE:
            if(bInitialized){
                winWidth=LOWORD(lParam); winHeight=HIWORD(lParam);
        ...
        case WM_KEYDOWN:
            if(wParam==VK_ESCAPE){
                fclose(gFILE);gFILE=NULL; DestroyWindow(hwnd);
            }
            break;
        case WM_CHAR:
            if(wParam=='f' || wParam=='F'){
                ToggleFullscreen(); gbFullscreen=!gbFullscreen;
            }
            break;
        case WM_CLOSE:
            DestroyWindow(hwnd);
...
static uint32_t instExtCount=0; static const char* instExts[4];
static uint32_t instLayerCount=0; static const char* instLayers[1];

static VkResult FillInstanceExtensions(void)
{
    uint32_t count=0; vkEnumerateInstanceExtensionProperties(NULL,&count,NULL);
    ...
    free(arr);
    if(!foundSurface||!foundWin32) return VK_ERROR_INITIALIZATION_FAILED;
    instExtCount=0;
...
{
    uint32_t c=0; vkEnumerateInstanceExtensionProperties(NULL,&c,NULL);
    if(!c) return;
    ...
    for(uint32_t i=0;i<c;i++){
        if(!strcmp(arr[i].extensionName,VK_EXT_DEBUG_UTILS_EXTENSION_NAME)){
            instExts[instExtCount++]=VK_EXT_DEBUG_UTILS_EXTENSION_NAME; break;
        }
    }
    free(arr);
}
...
{
    uint32_t c=0; vkEnumerateInstanceLayerProperties(&c,NULL);
    if(!c) return;
    ...
    for(uint32_t i=0;i<c;i++){
        if(!strcmp(arr[i].layerName,"VK_LAYER_KHRONOS_validation")){
            instLayers[0]="VK_LAYER_KHRONOS_validation"; instLayerCount=1; break;
        }
    }
    free(arr);
}
...
{
    VkResult r=FillInstanceExtensions(); if(r!=VK_SUCCESS) return r;
    TryAddDebugExt(); TryAddValidationLayer();
    VkApplicationInfo ai={VK_STRUCTURE_TYPE_APPLICATION_INFO};
    ai.pApplicationName=gpszAppName; ai.applicationVersion=1;
    ai.pEngineName=gpszAppName; ai.engineVersion=1;
    ...
    r=vkCreateInstance(&ici,NULL,&vkInstance); if(r!=VK_SUCCESS) return r;
    if(instLayerCount>0){
...
static VkResult CreateSurface(void)
{
    ...
    ci.hwnd=ghwnd;
    return vkCreateWin32SurfaceKHR(vkInstance,&ci,NULL,&vkSurfaceKHR);
}

static VkResult PickPhysicalDevice(void)
{
    uint32_t c=0; vkEnumeratePhysicalDevices(vkInstance,&c,NULL);
    ...
    int chosen=-1;
    for(uint32_t i=0;i<c;i++){
        uint32_t qc=0; vkGetPhysicalDeviceQueueFamilyProperties(devs[i],&qc,NULL);
        if(!qc) continue;
        ...
        for(uint32_t f=0;f<qc;f++){
            ...
            if((props[f].queueFlags & VK_QUEUE_GRAPHICS_BIT) && sup){
                ...
                graphicsQuequeFamilyIndex_selected=f; found=VK_TRUE; break;
            }
        }
        free(props);
...
static const char* devExts[2]; static uint32_t devExtCount=0;
static VkResult CheckDevExtensions(void)
{
    ...
    free(arr); devExtCount=0;
    if(!haveSwap) return VK_ERROR_INITIALIZATION_FAILED;
...
{
    VkResult r=CheckDevExtensions(); if(r!=VK_SUCCESS) return r;
    float qp=1.0f;
    VkDeviceQueueCreateInfo dqci={VK_STRUCTURE_TYPE_DEVICE_QUEUE_CREATE_INFO};
    dqci.queueFamilyIndex=graphicsQuequeFamilyIndex_selected;
    dqci.queueCount=1; dqci.pQueuePriorities=&qp;
...
{
    uint32_t c=0; vkGetPhysicalDeviceSurfaceFormatsKHR(vkPhysicalDevice_selected,vkSurfaceKHR,&c,NULL);
    if(!c) return VK_ERROR_INITIALIZATION_FAILED;
...
{
    uint32_t c=0; vkGetPhysicalDeviceSurfacePresentModesKHR(vkPhysicalDevice_selected,vkSurfaceKHR,&c,NULL);
    if(!c) return VK_ERROR_INITIALIZATION_FAILED;
    ...
    for(uint32_t i=0;i<c;i++){
        if(arr[i]==VK_PRESENT_MODE_MAILBOX_KHR){ vkPresentModeKHR=VK_PRESENT_MODE_MAILBOX_KHR; break;}
    }
    free(arr);
    return VK_SUCCESS;
}
...
{
    ChooseSwapFormat(); ChoosePresentMode();
    ...
    if(cap.currentExtent.width!=UINT32_MAX){
        ...
        if(vkExtent2D_SwapChain.width<cap.minImageExtent.width) vkExtent2D_SwapChain.width=cap.minImageExtent.width;
        if(vkExtent2D_SwapChain.height<cap.minImageExtent.height)vkExtent2D_SwapChain.height=cap.minImageExtent.height;
        if(vkExtent2D_SwapChain.width>cap.maxImageExtent.width) vkExtent2D_SwapChain.width=cap.maxImageExtent.width;
        if(vkExtent2D_SwapChain.height>cap.maxImageExtent.height)vkExtent2D_SwapChain.height=cap.maxImageExtent.height;
    }
    ...
    return vkCreateSwapchainKHR(vkDevice,&sci,NULL,&vkSwapchainKHR);
}
...
static VkResult buildCommandBuffers(void)
{
    for(uint32_t i=0;i<swapchainImageCount;i++){
        ...
        ri.renderArea.offset.x=0; ri.renderArea.offset.y=0;
...
static void cleanupSwapchain(void)
{
    ...
    if(vkCommandBuffer_array){
        ...
        free(vkCommandBuffer_array); vkCommandBuffer_array=NULL;
    }
    if(vkCommandPool){
    ...
    if(swapChainImageView_array){
        ...
        free(swapChainImageView_array); swapChainImageView_array=NULL;
    }
    if(swapChainImage_array){
        free(swapChainImage_array); swapChainImage_array=NULL;
    }
    if(vkSwapchainKHR){
...
static VkResult recreateSwapchain(void)
{
    ...
    cleanupSwapchain();
    VkResult r=CreateSwapchain(); if(r!=VK_SUCCESS)return r;
    r=CreateImagesViews(); if(r!=VK_SUCCESS)return r;
    r=CreateCommandPoolBuffers(); if(r!=VK_SUCCESS)return r;
...
void resize(int w,int h)
{
    ...
    if(winWidth&&winHeight) recreateSwapchain();
}
...
static VkResult drawFrame(void)
{
    ...
    uint32_t imgIndex;
    VkResult r=vkAcquireNextImageKHR(vkDevice,vkSwapchainKHR,UINT64_MAX,
        imageAvailableSemaphores[currentFrame],VK_NULL_HANDLE,&imgIndex);
...
{
    VkResult r=CreateVulkanInstance(); if(r!=VK_SUCCESS) return r;
    r=CreateSurface(); if(r!=VK_SUCCESS) return r;
    r=PickPhysicalDevice(); if(r!=VK_SUCCESS) return r;
    r=CreateDeviceQueue(); if(r!=VK_SUCCESS) return r;
    r=CreateSwapchain(); if(r!=VK_SUCCESS) return r;
    r=CreateImagesViews(); if(r!=VK_SUCCESS) return r;
    r=CreateCommandPoolBuffers(); if(r!=VK_SUCCESS) return r;
    r=CreateSyncObjects(); if(r!=VK_SUCCESS) return r;
...
VkResult display(void){ return drawFrame(); }
void update(void){}
void uninitialize(void)
{
    ...
    if(gFILE){fprintf(gFILE,"Program ended.\n"); fclose(gFILE); gFILE=NULL;}
}
...
int WINAPI WinMain(HINSTANCE hInst,HINSTANCE hPrev,LPSTR lpszCmdLine,int iCmdShow)
{
    ...
    wc.lpszMenuName=NULL; wc.lpszClassName=gpszAppName;
    wc.hIconSm=LoadIcon(hInst,MAKEINTRESOURCE(MYICON));
    RegisterClassEx(&wc);

    int sw=GetSystemMetrics(SM_CXSCREEN);
    int sh=GetSystemMetrics(SM_CYSCREEN);
    int x=(sw/2)-(WIN_WIDTH/2),y=(sh/2)-(WIN_HEIGHT/2);
    ...
    ghwnd=CreateWindowEx(
        WS_EX_APPWINDOW,gpszAppName,TEXT("DynamicRenderingNoLinkError"),
        WS_OVERLAPPEDWINDOW|WS_VISIBLE,x,y,WIN_WIDTH,WIN_HEIGHT,NULL,NULL,hInst,NULL);

    VkResult r=initialize(); if(r!=VK_SUCCESS){
        DestroyWindow(ghwnd); ghwnd=NULL;
    }
