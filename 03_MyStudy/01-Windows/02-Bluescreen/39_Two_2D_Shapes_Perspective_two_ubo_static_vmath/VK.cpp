...
static std::vector<Vertex> gVertices =
{
    ...
    // Right Square made up of 2 triangles (6 vertices)
    { { 0.25f, -0.5f }, { 1.0f, 1.0f, 0.0f } },   // Yellow
    { { 0.75f, -0.5f }, { 1.0f, 1.0f, 0.0f } },   // Yellow
    { { 0.25f,  0.5f }, { 1.0f, 1.0f, 0.0f } },   // Yellow
...
{
    // Keep the angle around the X-axis for the Triangle
    ...
    // Basic pipeline: model * view * proj
    // We will do a simple translation - but you can add rotation if you like.
    ...
    // Flip Y-axis for Vulkan (common approach)
    proj[1][1] *= -1.0f;

    // You can rotate about X if you want:
    // model = model * vmath::rotate(angleTriangle, 1.0f, 0.0f, 0.0f);

    // Then translate a bit on X / Z:
    vmath::mat4 trans = vmath::translate(-0.5f, 0.0f, -4.0f);

    ubo.mvp = proj * view * model * trans;
...
{
    // Keep the angle around the Y-axis for the Square
    ...
    // Optional rotation:
    // model = model * vmath::rotate(angleSquare, 0.0f, 1.0f, 0.0f);

    // Then place it at +0.5 X
    vmath::mat4 trans = vmath::translate(0.5f, 0.0f, -4.0f);
